...
| 004 | [Gate the license notice every source file carries](004-license-headers.md) | complete | A `license` subcommand: every source file carries the SPDX notice the repo declared |
...
| 006 | [Declare the gate set, and fail a repository that is missing one](006-gate-set-contract.md) | complete | A `contract` subcommand: the required gate set is compiled in, absence fails, exemptions carry a reason and a date |
...
| 011 | [The hooks hold every cheap gate, and pre-push lints what the push changes](011-hooks-hold-the-cheap-gates.md) | complete | `hook` adds goimports, license, and otel-client over staged files; a `prepush` runs golangci-lint on the packages a push changes; `contract` and `init` know the second hook |
...
| 014 | [One instrumented run of the suite carries test, race, cover, tempdir and hermetic](014-one-suite-run.md) | complete | A `suite` gate: vet, then one `go test -race` with a cover profile under the hermetic PATH inside the watched TMPDIR; the five gates fold into it in the plan, and CI runs one job where it ran five |
...
| 024 | [A failed rollout is history, not a veto on the next cut](024-a-failed-rollout-is-history.md) | complete | The previous tag's red Release run becomes a warning naming the run, the job and the actor; the window's push-triggered runs stay hard vetoes, and a green release run that published no GitHub Release still refuses |
| 026 | [A core names two audiences, its own name and the origin](026-a-core-names-two-audiences.md) | partial | `core-audiences`, an `identity` rule for the core role: the declared production overlay names exactly `identity.audience` and `api.latere.ai`; a hosted-plane name, a wrong count, a duplicate or a missing patch is a finding; arca and origo carry a dated waiver until 027 and 029 land |
| 025 | [A json parameter is not bytes, and the pooled endpoint is where that shows](025-json-parameters-are-not-bytes.md) | complete | `json-bytes`, a type-aware check in the `postgres` gate under every role: a byte slice bound into a parameter the statement casts to json, or carrying a json encoding, is the failure the pool reports and the direct endpoint hides; a `bytea` bind stays quiet |
