...
| 026 | [A core names two audiences, its own name and the origin](026-a-core-names-two-audiences.md) | draft | `core-audiences`, an `identity` rule for the core role: the declared production overlay names exactly `identity.audience` and `api.latere.ai`; a hosted-plane name, a wrong count, a duplicate or a missing patch is a finding; arca and origo carry a dated waiver until 027 and 029 land |
