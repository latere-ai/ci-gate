...
// Two entries can admit the same name. The reason printed has to be the same
// on every run, and it is the more specific entry's.
func TestTheLongestMatchingTempDirPrefixNamesTheReason(t *testing.T) {
	td := TempDir{Allow: map[string]string{
		"go-build":      "the toolchain cache",
		"go-build-work": "a go build -work under test",
		"unrelated":     "no",
	}}
	for range 200 {
		why, ok := td.AllowedFor("go-build-work4413")
		if !ok || why != "a go build -work under test" {
			t.Fatalf("AllowedFor = %q, %v; want the longest prefix's reason", why, ok)
		}
	}
}

func TestTheLongestMatchingCoverSuffixNamesTheReason(t *testing.T) {
	c := Cover{Exempt: map[string]string{
		"/gen":          "generated",
		"/internal/gen": "generated by the schema tool",
		"/other":        "no",
	}}
	for range 200 {
		why, ok := c.ExemptFor("example.com/m/internal/gen")
		if !ok || why != "generated by the schema tool" {
			t.Fatalf("ExemptFor = %q, %v; want the longest suffix's reason", why, ok)
		}
	}
}
