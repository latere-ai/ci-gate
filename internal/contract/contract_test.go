...
// repo is a tree in shape: caller, both hooks, gitignore, pin, and no Makefile.
...
func TestThePrepushMustDelegate(t *testing.T) {
	dir := repo(t)
	must(t, os.Remove(filepath.Join(dir, ".githooks", "pre-push")))
	_, err := check(t, dir, untracked(""))
	if err == nil || !strings.Contains(err.Error(), ".githooks/pre-push is missing; run `lateregate init`") {
		t.Fatalf("a missing pre-push names init, got %v", err)
	}

	write(t, dir, ".githooks/pre-push", "#!/bin/sh\n./scripts/changelog-for-tag.sh\n")
	must(t, os.Chmod(filepath.Join(dir, ".githooks", "pre-push"), 0o755))
	_, err = check(t, dir, untracked(""))
	if err == nil || !strings.Contains(err.Error(), "does not run `lateregate prepush`") {
		t.Fatalf("got %v", err)
	}

	// A repository's own check around the delegation is its own.
	write(t, dir, ".githooks/pre-push", "#!/bin/sh\nrefs=$(cat)\nprintf '%s\\n' \"$refs\" | go tool lateregate prepush || exit 1\necho \"$refs\" | ./scripts/changelog-for-tag.sh\n")
	must(t, os.Chmod(filepath.Join(dir, ".githooks", "pre-push"), 0o755))
	if _, err := check(t, dir, untracked("")); err != nil {
		t.Fatalf("a pre-push that delegates and does more is in shape: %v", err)
	}

	must(t, os.Chmod(filepath.Join(dir, ".githooks", "pre-push"), 0o644))
	_, err = check(t, dir, untracked(""))
	if err == nil || !strings.Contains(err.Error(), "pre-push is not executable") {
		t.Fatalf("a hook git cannot run is no hook, got %v", err)
	}
}

// A pre-push that exists and does not delegate holds a check somebody wrote,
// so init leaves it for a hand edit and contract keeps reporting it.
func TestInitLeavesAnOwnPrepushAlone(t *testing.T) {
	dir := repo(t)
	own := "#!/bin/sh\n./scripts/changelog-for-tag.sh\n"
	write(t, dir, ".githooks/pre-push", own)
	must(t, os.Chmod(filepath.Join(dir, ".githooks", "pre-push"), 0o755))
	var sb strings.Builder
	if err := Init(dir, &sb, untracked("")); err != nil {
		t.Fatal(err)
	}
	body, _ := os.ReadFile(filepath.Join(dir, ".githooks", "pre-push"))
	if string(body) != own {
		t.Error("init must not overwrite a pre-push holding the repository's own check")
	}
	if strings.Contains(sb.String(), "wrote .githooks/pre-push") {
		t.Error("init must not claim to have written it")
	}
	if _, err := check(t, dir, untracked("")); err == nil {
